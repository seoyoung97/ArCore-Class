...
public class MainRenderer implements GLSurfaceView.Renderer {
   ...
   // Sphere sphere;
    ...
    void addPoint(float x, float y, float z){
        ...
        //sphere.addNOCnt();
        sphere.currNO = (int)(Math.random()*5);
    ...
    void addLine(float [] end,float x, float y, float z){
        Line mLine = new Line(end,x,y,z, Color.GREEN);

        mLine.lineWidth = 300f;
        float [] matrix = new float[16];
        Matrix.setIdentityM(matrix,0);
        Matrix.translateM(matrix,0,x,y,z);

        mLine.setmModelMatrix(matrix);
